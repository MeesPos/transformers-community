<template>
    <nav class="my-16">
        <ol role="list" class="flex items-center justify-center">
            <li v-for="(step, stepIdx) in steps" :key="step.name" :class="[stepIdx !== steps.length - 1 ? 'pr-8 sm:pr-48' : '', 'relative']">
                <template v-if="step.status === 'complete'">
                    <div class="absolute inset-0 flex items-center" aria-hidden="true">
                        <div class="h-1 bg-brand-orange" :class="steps[steps.indexOf(step) + 1].status === 'current' ? 'w-8/12' : 'w-full'" />
                        <div class="h-1 bg-brand-purple" :class="steps[steps.indexOf(step) + 1].status === 'current' ? 'w-4/12' : 'hidden'" />
                    </div>
